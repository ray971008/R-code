...
m=length(names(CustomerChurn.dummy.x))+1 ## number of features+1 (including intercept)
del.y=0.50 ## delta
...
write.csv(be.simu, file="ZIB_beSimu cc.csv", row.names = F)
write.csv(del.simu, file="ZIB_DelSimu cc.csv", row.names = F)
write.csv(cutP.simu, file="ZIB_cutPSimu cc.csv", row.names = F)
write.csv(train.metric, file="ZIB_trainMetric cc.csv", row.names = F)
write.csv(test.metric, file="ZIB_testMetric cc.csv", row.names = F)
