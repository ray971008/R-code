...
CustomerChurn <- data.ecd(CustomerChurn.data,c(ord_var,num_var),fac_var)$factor

str(CustomerChurn)

table(CustomerChurn$Age.Group)
for (var in ord_var) {
  CustomerChurn[[var]] <- factor(CustomerChurn[[var]], ordered = TRUE)
}

xySomte <- smote_nc(CustomerChurn,"y")
str(xySomte)
table(xySomte$Age.Group)
for (var in ord_var) {
...
for (i in 1:iter){
  ...
  xySomte <- smote_nc(CustomerChurn,"y") # 插補
...
write.csv(metric.CBT.train, file = paste0("CBT_S1_trainMetric cc",".csv"), row.names = FALSE)
write.csv(metric.CBT.test, file = paste0("CBT_S1_testMetric cc",".csv"), row.names = FALSE)
