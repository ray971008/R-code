...
for (i in 1:iter){
  ...
  xtest <- data.test %>% select(-ytest)
  ytest <- data.test$ytest
...
write.csv(metric.ann.test, file = paste0("ann_S2_testMetric cc",".csv"), row.names = FALSE)
write.csv(metric.ann.train, file = paste0("ann_S2_trainMetric cc",".csv"), row.names = FALSE)
