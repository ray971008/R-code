...
for (i in 1:iter){
  # CBT.S1 with bank
  xySomte <- smote_nc(bank$factor,"y")
  x <- xySomte %>% select(-y)
  y <- xySomte$y
  dat.S=dataSplit(x,y)
...
write.csv(metric.CBT.test, file = paste0("CBT_S1_testMetric bank",".csv"), row.names = FALSE)
write.csv(metric.CBT.train, file = paste0("CBT_S1_trainMetric bank",".csv"), row.names = FALSE)
