...
# ==== 參數設定 ====
setwd('/Users/ray/Downloads/meeting/結果/bank/ZIB')
sink("ZIB_S1 bk.lst")

m=length(names(bank$dummy))-1+1 ## number of features+1 (including intercept)
del.y=0.50 ## delta
...
for (i in 1:iter){
  xySomte <- smote_nc(bank$factor,"y") # 插補
  xySomte <- data.ecd(xySomte,num,setdiff(cag, "y")) # 轉 dummy
  x <- xySomte$dummy %>% select(-y)
  y <- xySomte$dummy$y
  dat.S=dataSplit(x,y)
...
write.csv(be.simu, file="ZIB_S1_beSimu bk.csv", row.names = F)
write.csv(del.simu, file="ZIB_S1_DelSimu bk.csv", row.names = F)
write.csv(cutP.simu, file="ZIB_S1_cutPSimu bk.csv", row.names = F)
write.csv(train.metric, file="ZIB_S1_trainMetric bk.csv", row.names = F)
write.csv(test.metric, file="ZIB_S1_testMetric bk.csv", row.names = F)
