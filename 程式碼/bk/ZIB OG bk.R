...
m=length(names(bank.dummy.x))+1 ## number of features+1 (including intercept)
del.y=0.50 ## delta
...
write.csv(be.simu, file="ZIB_beSimu bk.csv", row.names = F)
write.csv(del.simu, file="ZIB_DelSimu bk.csv", row.names = F)
write.csv(cutP.simu, file="ZIB_cutPSimu bk.csv", row.names = F)
write.csv(train.metric, file="ZIB_trainMetric bk.csv", row.names = F)
write.csv(test.metric, file="ZIB_testMetric bk.csv", row.names = F)
