...
for (i in 1:iter){
  # CBT with bank
  dat.S=dataSplit(bank.factor.x,bank.factor.y)
...
write.csv(metric.CBT.test, file = paste0("CBT_testMetric bank",".csv"), row.names = FALSE)
write.csv(metric.CBT.train, file = paste0("CBT_trainMetric bank",".csv"), row.names = FALSE)
