...
# ==== 參數設定 ====
setwd('/Users/ray/Downloads/meeting/結果/spambase/ZIB')
sink("ZIB_S1 sp.lst")

m=length(names(spambase.dummy.x))+1 ## number of features+1 (including intercept)
del.y=0.50 ## delta
del.v=seq(0, 0.65, 0.05)  ## the domain to search delta
...
for (i in 1:iter){
  xySomte <- Smote(spambase.dummy.x,spambase.dummy.y)
...
write.csv(be.simu, file="ZIB_S1_beSimu sp.csv", row.names = F)
write.csv(del.simu, file="ZI_S1_DelSimu sp.csv", row.names = F)
write.csv(cutP.simu, file="ZIB_S1_cutPSimu sp.csv", row.names = F)
write.csv(train.metric, file="ZIB_S1_trainMetric sp.csv", row.names = F)
write.csv(test.metric, file="ZIB_S1_testMetric sp.csv", row.names = F)
