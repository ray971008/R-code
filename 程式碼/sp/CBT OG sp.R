...
for (i in 1:iter){
  # CBT with spambase
  dat.S=dataSplit(spambase.factor.x,spambase.factor.y)
...
write.csv(metric.CBT.test, file = paste0("CBT_testMetric spambase",".csv"), row.names = FALSE)
write.csv(metric.CBT.train, file = paste0("CBT_trainMetric spambase",".csv"), row.names = FALSE)
