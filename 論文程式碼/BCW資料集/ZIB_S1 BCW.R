...
m=length(names(BCW.x))+1 ## number of features+1 (including intercept)
del.y=0.50 ## delta
...
write.csv(be.simu, file="ZIB_S1_beSimu BCW.csv", row.names = F)
write.csv(del.simu, file="ZI_S1_DelSimu BCW.csv", row.names = F)
write.csv(cutP.simu, file="ZIB_S1_cutPSimu BCW.csv", row.names = F)
write.csv(train.metric, file="ZIB_S1_trainMetric BCW.csv", row.names = F)
write.csv(test.metric, file="ZIB_S1_testMetric BCW.csv", row.names = F)
